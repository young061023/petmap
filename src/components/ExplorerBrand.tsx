...
export function ExplorerBrand({ label = '반려견과 로컬 여행' }: { label?: string }) {
  return <View style={styles.brand}><View style={styles.wordmark}><Text style={styles.logo}>PetMap</Text><PawPrint size={18} color={colors.primary} /></View><View style={styles.destination}><Puppy size={38} /><Text style={styles.label}>{label}</Text></View></View>;
}
...
const styles = StyleSheet.create({
  ...
  wordmark: { flexDirection: 'row', alignItems: 'flex-start', gap: 3 }, logo: { fontSize: 27, letterSpacing: -1, fontWeight: '900', color: colors.primary },
