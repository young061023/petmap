        ...
        {isLoading ? <ActivityIndicator color={colors.primary} /> : <View style={styles.list}>
          {visible.map((achievement) => {
            ...
            return <View key={achievement.id} style={[styles.card, achievement.unlocked && styles.cardUnlocked]}>
              <View style={[styles.badge, !achievement.unlocked && styles.badgeLocked]}><Text style={styles.badgeIcon}>{achievement.unlocked ? achievement.icon : '🔒'}</Text></View>
...
const styles = StyleSheet.create({
  ...
  summaryIcon: { fontSize: 38 }, summaryCopy: { flex: 1 }, summaryValue: { color: colors.text, fontSize: 28, fontWeight: '800' },
  ...
  badgeLocked: { backgroundColor: colors.canvas }, badgeIcon: { fontSize: 28 }, cardCopy: { flex: 1, gap: 7 }, cardHeading: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
