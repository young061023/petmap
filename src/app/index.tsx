...
import {
  ...
  type MissionSegmentOption,
} from '@/components/mission/MissionSegmentedControl';
...
export default function MissionScreen() {
  ...
  const dailyMissions = useMemo(
    () => missions.filter((mission) => mission.period === 'daily'),
    [missions],
  );
  const visibleMissions = useMemo(
    () =>
      missions.filter(
        (mission) => mission.period === period && matchesMissionFilter(mission, filter),
      ),
    [filter, missions, period],
  );
  ...
  const openMission = (missionId: string) => {
    router.push({ pathname: '/missions/[missionId]', params: { missionId } });
  };
          ...
          <View style={styles.sectionHeader}>
            <View>
              ...
              <Text style={styles.sectionSubtitle}>
                {period === 'daily'
                  ? '매일 자정에 새로운 미션으로 바뀌어요.'
                  : '매주 월요일에 새로운 미션이 시작돼요.'}
              </Text>
            </View>
