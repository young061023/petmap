...
const CHARACTER_OPTIONS: { type: CharacterType; label: string; emoji: string }[] = [{ type: 'dog', label: '강아지', emoji: '🐶' }, { type: 'cat', label: '고양이', emoji: '🐱' }, { type: 'fox', label: '여우', emoji: '🦊' }];
  ...
  return <>
    <Pressable accessibilityRole="button" accessibilityLabel="지도 캐릭터 변경" onPress={() => setPickerVisible(true)} style={styles.characterCard}>
      ...
      <View style={styles.characterHero}>
        <Text style={styles.characterHeroEmoji}>{selected.emoji}</Text>
        <View style={{ flex: 1 }}>
